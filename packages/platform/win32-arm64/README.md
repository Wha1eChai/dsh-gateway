# dsh-gateway platform: win32 arm64

This private, data-only package carries the pinned CLIProxyAPI `v7.2.131`
Windows arm64 executable under `vendor/cli-proxy-api.exe`.

The package uses the literal upstream standard asset
`CLIProxyAPI_7.2.131_windows_aarch64.zip`. Dynamic plugins are disabled by the
...
runtime download. It is selected only on `win32`/`arm64` hosts.
