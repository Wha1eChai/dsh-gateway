...
export class FakeCpaServer extends EventEmitter {
  ...
  async start() {
    ...
    if (this.port !== 0 && FETCH_FORBIDDEN_PORTS.has(this.port)) {
      throw new TypeError(`port ${this.port} is forbidden by Fetch`);
    }
    for (let attempt = 0; attempt < 20; attempt += 1) {
      ...
      if (!FETCH_FORBIDDEN_PORTS.has(this.port)) return this;
      await this.close();
      this.port = 0;
    }
    throw new Error('fake CPA could not acquire a Fetch-safe loopback port');
  }
