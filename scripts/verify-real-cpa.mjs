...
async function verifyPlatformAsset() {
  ...
  assert(manifest.name === '@dshapps/dsh-gateway-platform-win32-x64', 'win32-x64 platform package name drifted')
  ...
  assert(isRecord(manifest.dshappsPlatform), 'win32-x64 platform metadata is missing')
  assert(typeof manifest.dshappsPlatform.binary === 'string', 'win32-x64 platform binary metadata is missing')
  assert(typeof manifest.dshappsPlatform.provenance === 'string', 'win32-x64 platform provenance metadata is missing')
  assert(typeof manifest.dshappsPlatform.managedConfig === 'string', 'win32-x64 platform config metadata is missing')
  assert(manifest.dshappsPlatform.provenance === 'provenance/cli-proxy-api.json', 'platform provenance path drifted')
  assert(manifest.dshappsPlatform.managedConfig === 'config/managed.yaml', 'platform managed config path drifted')
  ...
  const binaryPath = resolve(PLATFORM_ROOT, manifest.dshappsPlatform.binary)
